#include "SensorPM1006.hpp"

#define DEFAULT_TIMEOUT 1000

SensorPM1006::SensorPM1006(Stream& serial, bool debug) {
    _serial = &serial;
    _debug = debug;
...
bool SensorPM1006::init() {
    // Attempt one read to verify presence
    _available = read();
    return _available;
}

bool SensorPM1006::read() {
    uint16_t pm_value;
    if (read_pm25_internal(&pm_value)) {
        _pm25 = pm_value;
        return true;
    }
    return false;
...
bool SensorPM1006::read_pm25_internal(uint16_t* pm) {
...
bool SensorPM1006::send_command(size_t cmd_len, const uint8_t* cmd_data) {
...
int SensorPM1006::build_tx(size_t cmd_len, const uint8_t* cmd_data) {
    int len = 0;
    _txbuf[len++] = 0x11;
    _txbuf[len++] = cmd_len;
...
bool SensorPM1006::process_rx(uint8_t c) {
    switch (_state) {
        case PM1006_HEADER:
            _checksum = c;
            if (c == 0x16) {
